...
#WIN LOCAL
#ThisWeekFile = 'Weekly Reporting ' + ThisWeekDash + ' SDHY.xlsx'
#LastWeekFile = 'Weekly Reporting ' + LastWeekDash + ' SDHY.xlsx'
#DataPath = 'C:\Users\\jpardee\\Desktop\\JPY_Post\\II. Exposure Breakdown SDHY Backup '+ThisWeekDash+'.xlsx'
#InDir = 'C:\Users\\jpardee\\Desktop\\JPY_Post\\'
#OutPath = 'C:\Users\\jpardee\\Desktop\\JPY_Post\\' + ThisWeekDash + ' SDHY\\' + ThisWeekFile
#LastWeekPath = 'C:\Users\\jpardee\\Desktop\\JPY_Post\\' + LastWeekDash + ' SDHY\\' + LastWeekFile

#WIN DRIVE
ThisWeekFile = 'Weekly Reporting ' + ThisWeekDash + ' SDHY.xlsx'
LastWeekFile = 'Weekly Reporting ' + LastWeekDash + ' SDHY.xlsx'
DataPath = 'Y:/Non-US/NB East Asia (Japan & Korea)/Japan Post Bank/High Yield & Bank Loans/Weekly/Reports/SDSHY/2016/Backup/II. Exposure Breakdown SDHY Backup '+ThisWeekDash+'.xlsx'
#InDir = 'C:\Users\\jpardee\\Desktop\\JPY_Post\\'
OutPath = 'Y:/Non-US/NB East Asia (Japan & Korea)/Japan Post Bank/High Yield & Bank Loans/Weekly/Reports/SDSHY/2016/' + ThisWeekDash + ' SDHY/' + ThisWeekFile
LastWeekPath = 'Y:/Non-US/NB East Asia (Japan & Korea)/Japan Post Bank/High Yield & Bank Loans/Weekly/Reports/SDSHY/2016/' + LastWeekDash + ' SDHY/' + LastWeekFile
...
############################ EXPOSURE BREAKDOWN ############################
data = p.read_excel(io=DataPath, sheetname='JPHSH-JP SDHY-Risk and Exp', skiprows=6, skipfooter=3, index_col=0, parse_cols=6, na_values=['', ' '])
TotalDurCon = nm.float64(data.loc[['JPHSHY'], 'Duration Contribution'])
TotalSprdDurCon = nm.float64(data.loc[['JPHSHY'], 'Spread Duration Contribution'])
...
data.drop(labels=["US", "EMU", "UK", "Other", "JPHSHY"], inplace=True)
...
############################ YTW ############################
ReadYTW = p.read_excel(io=DataPath, sheetname='JPHSH-JP SDHY-Portfolio Gr', skiprows=6, skipfooter=3, index_col=0, parse_cols=1, na_values=['', ' '])

############################ CASH ############################
ReadCashMV = p.read_excel(io=DataPath, sheetname='JPHSH-JP HY --Risk and Exp', skiprows=6, skipfooter=4, index_col=0, parse_cols=2, na_values=['', ' '])

############################ By Sector SDHY ############################
Sectors = ['Aerospace/Defense', 'Airlines', 'Automotive & Auto Parts', 'Banking', 'Building Materials', 'Capital Goods', 'Chemicals', 'Consumer-Products', 'Diversified Financial Services', 'Energy', 'Environmental', 'Food & Drug Retail', 'Food Beverage & Tobacco', 'Gaming', 'Gas Distribution', 'Healthcare', 'Hotels', 'Insurance', 'Leisure', 'Media - Broadcast', 'Media - Diversified', 'Media - Cable', 'Metals/Mining', 'Packaging', 'Paper', 'Printing & Publishing', 'Railroads', 'Real Estate & Homebuilders', 'Restaurants', 'Steel', 'Super Retail', 'Support-Services', 'Technology & Electronics', 'Telecommunications', 'Theaters & Entertainment', 'Transportation Excluding Air/Rail', 'Utilities']

ReadLastSec = p.read_excel(io=LastWeekPath, sheetname='II. By sector for SDHY', skiprows=2, skip_footer=0, index_col=0, header=0, parse_cols='B:G')
ReadLastSec.dropna(axis=0, inplace=True)

ReadSec = p.read_excel(io=DataPath, sheetname='JPHSH-JP SDHY-Spread Durat', skiprows=6, skipfooter=3, index_col=0, parse_cols='A:F')
...
############################ WRITE By Sector SDHY ############################
#LAST WEEK TABLE VALUES
ws = wb.get_sheet_by_name('II. By sector for SDHY')
LastSecBegRow = 6
LastSecBegCol = 8
...
wb.save(OutPath)
Backupwb = xl.load_workbook('Y:/Non-US/NB East Asia (Japan & Korea)/Japan Post Bank/High Yield & Bank Loans/Weekly/Reports/SDSHY/2016/Backup/II. Exposure Breakdown SDHY Backup '+ThisWeekDash+'.xlsx')
Backupwb.save(OutPath[:OutPath.rfind('/')] + '/Backup/' + 'II. Exposure Breakdown SDHY Backup '+ThisWeekDash+'.xlsx')
